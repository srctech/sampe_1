...
class MathSolution:
    ...
    def math_allocation(self, assets_and_pools):
        ...
        for k, v in converted['pools'].items():
            base_rate, base_slope, kink_slope, optimal_util_rate = v['base_rate'], v['base_slope'], v['kink_slope'], v[
                'optimal_util_rate']
            utilization_rate = v['borrow_amount'] / v['reserve_size']
            v['predicted_apy'] = self.calculate_apy(base_rate, base_slope, kink_slope, optimal_util_rate,
                                                    utilization_rate)
